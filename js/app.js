...
Enemy.prototype.checkCollisions = function(dt) {
  this.eDim = {
    x: this.x = this.x + this.speed * dt,
    y: this.y,
    w: 73,
    h: 54
  };
  if (player.x < this.eDim.x + this.eDim.w && player.x + player.w > this.eDim.x && player.y < this.eDim.y + this.eDim.h && player.h + player.y > this.eDim.y) {
    player.reset();
    alert("**test** you died!");
  }
};
